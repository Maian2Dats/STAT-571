...
```{r read data}
surveyres <- read.csv("D:/Courses/2021 Spring/STAT 571/Homework/Home work 1/data/Survey_results_final.csv", header=T, stringsAsFactors = FALSE)  
head(surveyres)
# names(surveyres) # see what variables
```
...
```{r warning=FALSE}
# surveyres$age
surveyres$age <- as.numeric(surveyres$age)
surveyres$age[surveyres$age>100] <- median(surveyres$age[surveyres$age<=100 & which(!is.na(surveyres$age))]) # replace outliers with median
surveyres$age[is.na(surveyres$age)] <- median(surveyres$age[which(!is.na(surveyres$age))]) # replace na with median
cat("max age:", max(surveyres$age), "\n")
cat("min age:", min(surveyres$age), "\n")
cat("null count:", sum(is.na(surveyres$age)), "\n")
```

```{r}
# surveyres$gender
surveyres$gender[!surveyres$gender %in% c("Male","Female")] # detect abnormal values
surveyres$gender[!surveyres$gender %in% c("Male","Female")] <- tail(names(sort(table(surveyres$gender))), 1) # replace with most frequent value
unlist(distinct(surveyres, gender))
```
```{r}
# surveyres$education
cat("null count:", sum(is.na(surveyres$education)))
```
```{r}
# surveyres$income
cat("empty count:", sum((surveyres$income == "")))
surveyres$income[surveyres$income == ""] <- tail(names(sort(table(surveyres$income))), 1) # reaplace with most frequent value
distinct(surveyres, income)
unlist(distinct(surveyres, gender))
```
```{r}
# surveyres$sirius
surveyres$sirius[!surveyres$sirius %in% c("Yes","No")] # detect abnormal values
surveyres$sirius[!surveyres$sirius %in% c("Yes","No")] <- tail(names(sort(table(surveyres$sirius))), 1) # replace with most frequent value
unlist(distinct(surveyres, sirius))
```
```{r}
# surveyres$wharton
surveyres$wharton[!surveyres$wharton %in% c("Yes","No")] # detect abnormal values
surveyres$wharton[!surveyres$wharton %in% c("Yes","No")] <- tail(names(sort(table(surveyres$sirius))), 1) # replace with most frequent value
unlist(distinct(surveyres, wharton))
```

```{r}
# surveyres$worktime
cat("max worktime:", max(surveyres$worktime), "\n")
cat("min worktime:", min(surveyres$worktime), "\n")
cat("null worktime:", sum(is.na(surveyres$worktime)), "\n")
```
...
```{r}
hist(surveyres$age)
hist(surveyres$worktime)
pie(table(surveyres$sirius), col=grey.colors(2), main="Sirius? Distribution")
pie(table(surveyres$wharton), col=grey.colors(2), main="Wharton? Distribution")
pie(table(surveyres$gender), col=grey.colors(2), main="Gender Distribution")
pie(table(surveyres$income), col=grey.colors(6), main="Income Distribution")
```
```{r}
ggplot(surveyres, aes(x = gender, fill = sirius)) +
    geom_bar(position = position_dodge()) +
    theme_classic()

ggplot(surveyres, aes(x = income, fill = sirius)) +
    geom_bar(position = position_dodge()) +
    theme_classic()
```
## Sample properties
...
```{r}
cat("Sirius listener count:", sum(surveyres$sirius == "Yes"), "\n")
cat("Wharton listener count:", sum(surveyres$wharton == "Yes"), "\n")
cat("Final estimate of the Wharton audience size:", sum(surveyres$wharton == "Yes") / sum(surveyres$sirius == "Yes") * 51.6 * 1000000)
```


* Goal of the study: estimate of the Wharton audience size in January 2014.
* Assumptions: 
  + the sample is a random sample of the MTURK population.
  + the proportion of Wharton listeners vs. Sirius listeners in the general population is the same as that in the MTURK population.
* Formula: Wharton audience size = (Wharton audience size in sample / Sirius audience size in sample) * Sirius audience size
* Findings: Wharton audience size = (74 / 1365) * 51.6M = 2,797,363
...
increase pay
extend data collecting period by limit survey / day
...
```{r}
wsci <- read_excel("D:/Courses/2021 Spring/STAT 571/Homework/Home work 1/data/WomenData_06_16.xlsx")
head(womends)
```
```{r}
#change names
...
wsci %<>% 
  mutate(Field = as.factor(Field), Degree = as.factor(Degree), Sex = as.factor(Sex), Year = as.integer(Year), Number = as.integer(Number))
attach(wsci)
head(wsci)
```
```{r}
cat("count of missing values:", sum(is.na(wsci)), "\n")
cat("number of fields:", length(names(wsci)))
...
```{r read data}
mlbase <- read.csv("D:/Courses/2021 Spring/STAT 571/Homework/Home work 1/data/baseball.csv", header=T, stringsAsFactors = FALSE)  
head(mlbase)
# names(mlbase) # see what variables
```
 **option 1** indicates the absolute increment of payroll while **option 2** indicates the relative increment of payroll. Since we care more about the amount that payroll increased from last year, that is the increment percentage, log difference is more appropriate in this setup.

```{r}
mlbase %<>%
  group_by(team) %>%
  mutate(diff_log = log(payroll) - log(lag(payroll, 1))) %>%
...
mlbase %<>%
  group_by(team) %>%
  mutate(diff_win_pct = win_pct - lag(win_pct, 1))
head(mlbase)
```
...
attach(mlbase)
mlbase %>%
  filter(!is.na(diff_log) & !is.na(diff_win_pct)) %>%
  ggplot(aes(x=diff_log, y=diff_win_pct, group = team, color=team)) +
  geom_point()+
...
mlbase %>%
  filter(!is.na(diff_log) & !is.na(diff_win_pct)) %>%
  ggplot(aes(x=diff_log, y=diff_win_pct, group = year, color=team)) +
  geom_point()+
  geom_smooth(method="lm", formula=y~x, se=F,color = "red")+
  facet_wrap(~year) + 
  theme_bw() +
